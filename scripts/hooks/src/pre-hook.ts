...
async function main() {
  ...
  const options: PreHookOptions = program.opts();
...
async function runPreHookValidation(options: PreHookOptions, config: any): Promise<ValidationResult> {
  ...
  for (const filePath of filesToValidate) {
    try {
      ...
      const filteredViolations = result.violations.filter(v => {
        if (options.severity === 'all') return true;
        if (options.severity === 'warning') return v.level === 'warning' || v.level === 'error';
...
async function attemptAutoFix(violations: Violation[], config: any): Promise<number> {
  let fixedCount = 0;
  
  // Group violations by file
  const violationsByFile = violations.reduce((acc, violation) => {
    if (!violation.file) return acc;
...
async function tryFixViolation(violation: Violation, content: string): Promise<string | null> {
  ...
  switch (violation.rule) {
    case 'lever-leverage-patterns':
      if (violation.message.includes('LoadingDialog')) {
        ...
        return content.replace(
          /<div className="spinner">Loading\.\.\.<\/div>/g,
          '<LoadingSpinner />'
        );
      }
      break;
    ...
    case 'technical-documentation':
      if (violation.message.includes('JSDoc documentation')) {
        ...
        if (violation.line) {
          lines.splice(violation.line - 1, 0, 
            '/**',
