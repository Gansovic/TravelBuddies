...
import { loadConfig, shouldIgnoreFile, printResults } from './lib/utils.js';
...
async function runPostHookValidation(options, config) {
...
async function validateStagedFiles(options, config) {
    console.log(chalk.blue('📁 Validating staged files...'));
    // Get staged files
    const stagedFiles = getStagedFiles().filter(f => existsSync(f) && !shouldIgnoreFile(f, config));
...
async function validateCommit(options, config) {
...
async function validateTestCoverage(options, config) {
    console.log(chalk.blue('🧪 Validating test coverage...'));
    const violations = [];
    try {
        // Check if new files have corresponding tests
        const stagedFiles = getStagedFiles();
        const newCodeFiles = stagedFiles.filter(f => (f.endsWith('.ts') || f.endsWith('.tsx')) &&
            !f.includes('.test.') &&
            !f.includes('.spec.') &&
            !f.includes('/test/') &&
...
async function validateAuditLogging(options, config) {
    ...
    for (const filePath of stagedFiles) {
        ...
        try {
            ...
            const hasValueOperations = valueOperationPatterns.some(pattern => pattern.test(content));
...
function findTestFile(codeFile) {
