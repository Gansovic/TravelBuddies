...
export class ExportService {
     ...
     */
    async exportAsZip(tripId, options = {}) {
        try {
     ...
     */
    async exportAsPDF(tripId, options = {}) {
        try {
            ...
            for (const [date, dayMoments] of Object.entries(dayGroups)) {
                ...
                pdf.setFontSize(10);
                pdf.text(`${dayMoments.length} moments`, 20, 30);
                let yPosition = 45;
                // Add moments for this day
                for (const moment of dayMoments.slice(0, 10)) {
                    if (yPosition > 250) {
                        pdf.addPage();
                        yPosition = 20;
                    }
        ...
        try {
            ...
            const exportData = {
                ...
                metadata: {
                    total_moments: moments.length,
                    date_range: {
                        start: moments.length > 0 ? Math.min(...moments.map((m) => new Date(m.captured_at).getTime())) : null,
                        end: moments.length > 0 ? Math.max(...moments.map((m) => new Date(m.captured_at).getTime())) : null
                    },
                    locations_visited: Array.from(new Set(moments.map((m) => m.city).filter(Boolean))),
                    moment_types: Array.from(new Set(moments.map((m) => m.type)))
                }
            };
            return exportData;
        }
    ...
    getFileExtension(momentType, mimeType) {
    ...
    generateGPX(geoMoments, tripId) {
        const waypoints = geoMoments.map((moment) => `
    <wpt lat="${moment.latitude}" lon="${moment.longitude}">
      ...
      <type>${moment.type}</type>
        ...
        const trackPoints = geoMoments.map((moment) => `
...
<gpx version="1.1" creator="TravelBuddies" xmlns="http://www.topografix.com/GPX/1/1">
  ...
  <trk>
    ...
    async calculateTripStats(moments) {
        const geoMoments = moments.filter((m) => m.latitude && m.longitude);
        return {
    ...
    calculateDurationDays(moments) {
        if (moments.length === 0)
            return 0;
        const dates = moments.map((m) => (m.captured_at?.toISOString?.() || m.captured_at).split('T')[0]);
        const uniqueDates = Array.from(new Set(dates));
        return uniqueDates.length;
    }
    calculateTotalDistance(geoMoments) {
        if (geoMoments.length < 2)
            return 0;
        let totalDistance = 0;
