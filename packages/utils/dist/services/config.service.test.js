...
describe('ConfigService', () => {
    ...
    describe('getConfig', () => {
        it('should fetch from cache first', async () => {
            ...
            vi.spyOn(service, 'getDefault').mockReturnValue(cachedValue);
        ...
        it('should throw error for unknown config key', async () => {
            // Act & Assert
            await expect(configService.getConfig('unknown_key')).rejects.toThrow("Configuration key 'unknown_key' not found");
        });
    });
    ...
    describe('clearCache', () => {
        it('should clear memory cache', () => {
            // Arrange
            const service = ConfigService.getInstance();
            service.memoryCache.set('test', 'value');
            // Act
            service.clearCache();
            // Assert
