...
export class MomentService {
     ...
     */
    async getMoments(tripId, filters) {
        ...
        `)
            ...
            return data?.map((moment) => this.convertDatabaseMoment(moment)) || [];
        }
        catch (error) {
     ...
     * Update a moment
     */
    async updateMoment(momentId, input) {
        if (!this.supabase || !momentId)
            return false;
        try {
     ...
     */
    async addReaction(momentId, emoji) {
        if (!this.supabase || !momentId || !emoji)
            return false;
        try {
     ...
     */
    async removeReaction(momentId, emoji) {
        if (!this.supabase || !momentId || !emoji)
            return false;
        try {
            const userId = (await this.supabase.auth.getUser())?.data?.user?.id;
    ...
    async getTimeline(tripId) {
        ...
        try {
            ...
            const days = Array.from(dayMap.entries()).map(([date, dayMoments]) => {
                const cities = [...new Set(dayMoments.map(m => m.city).filter(Boolean))];
                ...
                for (let i = 1; i < dayMoments.length; i++) {
                    ...
                    if (prev.latitude && prev.longitude && curr.latitude && curr.longitude) {
                        distance_km += this.calculateDistance(prev.latitude, prev.longitude, curr.latitude, curr.longitude);
                    }
                }
                return {
                    date,
     ...
     */
    subscribeToMoments(tripId, callback) {
        if (!this.supabase || !tripId)
            return () => { };
     ...
     */
    async uploadMediaAsync(momentId, file) {
        if (!this.supabase)
            return;
        try {
     ...
     */
    async enhanceLocationDataAsync(momentId, latitude, longitude) {
        try {
