...
export class TripService {
    ...
    async getUserTrips(userId) {
        ...
        `)
            ...
            const tripIds = data.map((item) => item.trip_id);
        ...
        `)
            ...
            return {
                ...
                total_stats: {
                    ...
                    countries_visited: [...new Set(moments?.map((m) => m.country).filter(Boolean) || [])],
                    cities_visited: [...new Set(moments?.map((m) => m.city).filter(Boolean) || [])]
                }
            };
        }
        catch (error) {
    ...
    async getTripStats(userId) {
        ...
        `)
            ...
            const totalMoments = trips.reduce((sum, trip) => sum + (trip.trip_timeline?.total_moments || 0), 0);
            const allCountries = new Set();
            const allCities = new Set();
            let totalDistance = 0;
            trips.forEach((trip) => {
                trip.trip_timeline?.countries_visited?.forEach((country) => allCountries.add(country));
                trip.trip_timeline?.cities_visited?.forEach((city) => allCities.add(city));
     ...
     */
    async getRecentActivity(tripId, limit = 20) {
        if (!this.supabase || !tripId)
            return [];
        try {
            const { data, error } = await this.supabase
     ...
     */
    async inviteMember(tripId, input) {
        if (!this.supabase || !tripId)
            return false;
        try {
            const { error } = await this.supabase
    ...
    convertDatabaseTrip(dbTrip) {
        return {
            ...
            members: dbTrip.trip_members?.map((member) => ({
                id: member.id,
                trip_id: dbTrip.id,
                user_id: member.user_id,
                role: member.role,
    ...
    convertDatabaseMoment(dbMoment) {
        return {
            ...
            reactions: dbMoment.moment_reactions?.map((reaction) => ({
                ...reaction,
