...
export class OfflineStorageService {
  ...
  private dbName = 'TravelBuddiesDB';
  private dbVersion = 1;
   ...
   */
  async queueMoment(momentData: CreateMomentInput & { id: string }): Promise<boolean> {
   ...
   * Cache moments locally for fast access
   */
  async cacheMoments(moments: Moment[]): Promise<boolean> {
    if (!this.db) return false;
    ...
    try {
      ...
      await Promise.all(moments.map(moment => 
   ...
   */
  async getCachedMoments(tripId: string): Promise<Moment[]> {
    if (!this.db) return [];
   ...
   */
  async storeMediaFile(momentId: string, file: File): Promise<boolean> {
    if (!this.db) return false;
   ...
   */
  async getMediaFile(momentId: string): Promise<File | null> {
   ...
   */
  async clearOldCache(daysToKeep: number = 7): Promise<boolean> {
    if (!this.db) return false;
