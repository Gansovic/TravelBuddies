...
export class TripService {
   ...
   */
  async getTrip(tripId: string): Promise<any | null> {
   ...
   */
  async searchTrips(userId: string, query: string, limit: number = AppConstants.defaultPageSize): Promise<any[]> {
    if (!this.supabase || !userId || !query.trim()) return [];
   ...
   */
  async getRecentTrips(userId: string, limit: number = 5): Promise<any[]> {
    if (!this.supabase || !userId) return [];
   ...
   */
  async getTripsByDateRange(userId: string, startDate: string, endDate: string): Promise<any[]> {
    if (!this.supabase || !userId) return [];
   ...
   */
  async hasAccessToTrip(userId: string, tripId: string): Promise<boolean> {
    if (!this.supabase || !userId || !tripId) return false;
