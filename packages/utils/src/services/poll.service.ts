...
export class PollService {
   ...
   */
  async getPolls(tripId: string, options?: PollSearchOptions): Promise<Poll[]> {
    ...
    try {
      ...
      return data?.map((poll: any) => {
        const convertedPoll = this.convertDatabasePoll(poll);
        convertedPoll.options = poll.poll_options?.map((opt: any) => {
          const convertedOption = this.convertDatabasePollOption(opt);
          convertedOption.votes = opt.poll_votes?.map((vote: any) => this.convertDatabasePollVote(vote)) || [];
  ...
  async getPoll(pollId: string): Promise<Poll | null> {
    if (!this.supabase || !pollId) return null;
    ...
    try {
      ...
      poll.options = data.poll_options?.map((opt: any) => {
        const option = this.convertDatabasePollOption(opt);
        option.votes = opt.poll_votes?.map((vote: any) => this.convertDatabasePollVote(vote)) || [];
        return option;
      }) || [];
   ...
   */
  async vote(input: VotePollInput): Promise<boolean> {
  ...
  async removeVote(pollId: string, userId?: string): Promise<boolean> {
    if (!this.supabase || !pollId) return false;
   ...
   */
  async closePoll(pollId: string): Promise<PollResults | null> {
    if (!this.supabase || !pollId) return null;
  ...
  subscribeToPolls(tripId: string, callback: (poll: Poll) => void): () => void {
    ...
    const subscription = this.supabase
      ...
      .on('postgres_changes', {
        ...
        if (poll && poll.trip_id === tripId) callback(poll);
      })
      .subscribe();
   ...
   */
  private convertDatabasePoll(dbPoll: DatabasePoll): Poll {
    return {
      id: dbPoll.id,
   ...
   */
  private convertDatabasePollOption(dbOption: DatabasePollOption): PollOption {
    return {
      id: dbOption.id,
   ...
   */
  private convertDatabasePollVote(dbVote: DatabasePollVote): PollVote {
    return {
      id: dbVote.id,
