...
export class MetadataExtractionService {
   ...
   */
  async extractLocationMetadata(latitude: number, longitude: number): Promise<LocationMetadata> {
   ...
   */
  async reverseGeocode(latitude: number, longitude: number): Promise<PlaceData | null> {
    try {
  ...
  async getTimezone(latitude: number, longitude: number): Promise<string> {
    try {
      ...
      const response = await fetch(
        `https://maps.googleapis.com/maps/api/timezone/json?location=${latitude},${longitude}&timestamp=${Math.floor(Date.now() / 1000)}&key=${process.env.GOOGLE_PLACES_API_KEY}`
      );
   ...
   */
  async analyzeContent(type: string, mediaUrl?: string, text?: string): Promise<ContentAnalysis> {
    try {
   ...
   */
  private extractTextTags(text: string): string[] {
    const tags: string[] = [];
   ...
   */
  async enhanceMoment(momentData: any): Promise<any> {
    const enhanced = { ...momentData };

    // Add location metadata if coordinates are available
    if (momentData.latitude && momentData.longitude) {
      const locationMetadata = await this.extractLocationMetadata(
        momentData.latitude,
        momentData.longitude
      );
   ...
   */
  private extractCity(placeData: any): string | undefined {
    if (!placeData?.address_components) return undefined;
   ...
   */
  private extractRegion(placeData: any): string | undefined {
    if (!placeData?.address_components) return undefined;
   ...
   */
  private extractCountry(placeData: any): string | undefined {
    if (!placeData?.address_components) return undefined;

    const countryComponent = placeData.address_components.find((component: any) =>
  ...
  private getEmptyLocationMetadata(): LocationMetadata {
    return {
      place_types: []
    };
  }
