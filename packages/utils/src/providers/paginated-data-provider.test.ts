...
import { PaginatedDataProvider, PaginationOptions, PaginatedResult } from './paginated-data-provider';

// Test implementation of PaginatedDataProvider
class TestPaginatedProvider extends PaginatedDataProvider<{ id: number; name: string }> {
  private mockData: { id: number; name: string }[] = [];
  private mockTotalCount = 0;

  constructor(data: { id: number; name: string }[] = [], totalCount = 0) {
    super();
    this.mockData = data;
    this.mockTotalCount = totalCount;
  }

  protected async fetchPage(options: PaginationOptions): Promise<PaginatedResult<{ id: number; name: string }>> {
...
describe('PaginatedDataProvider', () => {
  let provider: TestPaginatedProvider;
  ...
  describe('getData', () => {
    ...
    it('should use cache for repeated requests', async () => {
      const fetchPageSpy = vi.spyOn(provider as any, 'fetchPage');
